...
# 1. Make the title-scoped DeviceShared transfer-memory workaround symmetric.
...
# 2. Prevent an unmatched nvmap unpin from driving the pin count negative.
...
replace_function(
    ...
    if (handle_description->pins <= 0) {
        LOG_WARNING(Service_NVDRV,
                    "V18B_COMPAT ignoring unmatched nvmap unpin for handle={}", handle);
...
wrapper_text = wrapper_h.read_text()
color_signature = "    void ClearColorImage(VkImage image, VkImageLayout layout, VkClearColorValue color,"
color_start = wrapper_text.find(color_signature)
if color_start < 0:
    raise RuntimeError("add command-buffer depth-stencil clear: ClearColorImage not found")
color_brace = wrapper_text.find("{", color_start)
depth = 0
color_end = None
for index in range(color_brace, len(wrapper_text)):
    if wrapper_text[index] == "{":
        depth += 1
    elif wrapper_text[index] == "}":
        depth -= 1
        if depth == 0:
            color_end = index + 1
            if color_end < len(wrapper_text) and wrapper_text[color_end] == "\n":
                color_end += 1
            break
if color_end is None:
    raise RuntimeError("add command-buffer depth-stencil clear: ClearColorImage end not found")
...
'''
wrapper_h.write_text(wrapper_text[:color_end] + clear_depth_method + wrapper_text[color_end:])
print(f"updated {wrapper_h}: add command-buffer depth-stencil clear")
...
# 4. Eden currently skips uploads to multisampled depth/stencil images. Initialize the image to a
# valid depth/stencil value rather than leaving it undefined. The existing color-MSAA path remains
# unchanged.
...
new_msaa_fallback = '''    if (info.num_samples > 1) {
        ...
        if (is_depth_stencil) {
            ...
            LOG_WARNING(Render_Vulkan,
                        "V18B_COMPAT initialized unsupported MSAA depth/stencil format {} by clear",
                        info.format);
        } else {
            LOG_WARNING(Render_Vulkan, "MSAA upload not implemented for format {}", info.format);
        }
